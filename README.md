...
\*Please refer to the process diagram:\* `Process\_Diagram\_Rotameter.png`



\## 3.0 Control Philosophy

The control system calculates the flow rate based on the following logic and mathematical scaling:
...
3\. \*\*Flow Calculation:\*\* Upon receiving a pulse, the logic updates the flow rate using the following mathematical transfer function:
...
\* $Q$ = Volumetric Flow Rate in GPM (`F8:0`)

\* $k$ = Rotameter k-factor (6.3 gallons/pulse)

\* $t$ = Pulse time interval in seconds
